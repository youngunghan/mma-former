...
- 주요 서브모듈: `qkv` `Linear(dim,3·dim)`, `proj` `Linear(dim,dim)`, `router`(`num_routed_heads>0`일 때) `Linear(dim,dim/4)→ReLU→Linear(dim/4,num_routed_heads)`, `alpha_router` `Linear(dim,2)`.
...
| `combined_loss_fn` | `(logits, labels, load_balance_loss, weight=0.005)` | `(total, bce, lb)`. `total = BCEWithLogitsLoss(pos_weight=1.5) + weight·lb` |
